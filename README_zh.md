...
- 全局来源：`~/.pi/agent/workspaces/<名字>.json`
- 项目来源：`<repo>/.pi/workspaces/<名字>.json`
...
- `promptInOtherDirs`：会话在其他任何目录启动时，把该工作区列进加载询问清单。
...
`session_start` 时扩展扫描两个定义来源，然后：

1. **自动加载**：如果会话目录等于某工作区的主根，且其 `autoLoadInPrimary` 解析为真，立即加载该工作区。
2. **日志恢复**：如果会话是 `/resume` 恢复而来，重新激活上次记录的工作区。
3. **询问**：否则，只要存在任何定义且会话有 UI，pi 会弹出选择列表（带"不加载"逃生项）。列表分两层：当会话目录位于某工作区的*非主根*内时，只列出包含该目录的工作区；否则列出所有 `promptInOtherDirs` 解析为真的工作区。
...
| `/workspace create <名字>` | 在**全局**来源创建定义，以当前目录为唯一主根，并激活它。 |
| `/workspace add-root [名字] <路径>` | 给当前工作区添加根。名字可省略（默认取目录 basename）；相对路径锚定在会话目录。定义会持久化写回其来源。 |
| `/workspace remove-root <名字>` | 从当前工作区移除根并持久化。主根不可移除。 |
...
两个来源在 `session_start` 时扫描并**按名称合并**：同名时项目定义覆盖全局定义 —— 项目来源按名胜出。永远不会发生整体覆盖：项目文件无法遮蔽你无关的全局工作区，它们会并行加载。
...
项目级安装仅在会话从该仓库内启动时加载。从其他任何目录启动的会话都不会加载扩展，自动加载、询问弹窗、工具覆盖和状态栏全部缺席 —— 包括从同一工作区另一个根启动的会话。这是预期行为而非 bug：pi 从会话自身目录发现项目扩展。希望扩展随处可用请用全局安装。

## 运行时存储位置

| 内容 | 位置 |
|------|------|
| 全局定义 | `~/.pi/agent/workspaces/*.json` |
| 项目定义 | `<repo>/.pi/workspaces/*.json` |
| 全局默认选项 | `~/.pi/agent/pi-workspaces.json` |
