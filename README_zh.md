...
- **编辑器补全。** 在输入框敲 `@` 会列出根切换项和当前根（会话目录所在的根）下的文件 —— 选择根名即进入该根逐级补全，也可以不点名根、直接输入当前根内的路径；所有补全项接受后都插入显式的 `@根名/路径` 形式。`/workspace` 命令同样支持参数补全：子命令、`load` 的工作区名（排除当前激活的）、`remove` 的根名、`add` 的文件系统路径。其他输入一律委托给 pi 的内置补全。（补全只服务于你手动输入的路径 —— 模型生成工具调用路径时不经过它。）
- **底部状态栏。** 状态栏常驻显示当前工作区：`[ws] <workspace> (3 roots)`。当某个根目录在磁盘上缺失时状态会降级显示：`[ws] <workspace> (2/3 roots) ! <root> missing`。没有激活工作区时该条目自动清除。
